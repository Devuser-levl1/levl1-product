...
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  try {
    ...
    if (!approval || approval.type !== 'client_rubric') {
      return NextResponse.json({ error: 'Invalid rubric approval link' }, { status: 404 })
    }
    ...
    return NextResponse.json({
      ...
      l2ScoreThreshold: pos.l2ScoreThreshold ?? 75,
      scoringRubric:    pos.scoringRubric ?? { technical: 40, problemSolving: 30, behavioral: 20, eq: 10 },
