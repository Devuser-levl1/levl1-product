  ...
  return (
    <div style={{ maxWidth: 820 }}>
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #E2E8F0', marginBottom: 18 }}>
        {(['Upcoming', 'Past', 'Cancelled'] as const).map((t) => <button key={t} onClick={() => setTab(t)} style={{ padding: '10px 16px', fontSize: 14, fontWeight: 600, background: 'none', border: 'none', borderBottom: '2px solid ' + (tab === t ? '#6D28D9' : 'transparent'), color: tab === t ? '#6D28D9' : '#64748B', cursor: 'pointer' }}>{t}</button>)}
      </div>

      {filtered.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: '#475569' }}>No {tab.toLowerCase()} interviews.</div>}

      {order.filter((k) => groups[k]?.length).map((k) => (
        <div key={k} style={{ marginBottom: 20 }}>
          {tab === 'Upcoming' && <div style={{ fontSize: 12, fontWeight: 800, color: k === 'Overdue' ? '#D97706' : '#475569', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{k}</div>}
                ...
                <div key={i.id} style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                      ...
                      {i.scorecard?.overall && <span style={{ fontSize: 10, fontWeight: 700, color: '#6D28D9', background: 'rgba(109,40,217,0.08)', borderRadius: 100, padding: '2px 8px' }}>Scorecard: {i.scorecard.overall.replace('_', ' ')}</span>}
                  ...
                  {i.meetLink && i.status === 'SCHEDULED' && <a href={i.meetLink} target="_blank" rel="noreferrer" style={{ ...inp, fontWeight: 600, color: '#6D28D9', textDecoration: 'none' }}>Join</a>}
