...
function ImportModal({ jobs, onClose, onDone }: { jobs: Job[]; onClose: () => void; onDone: () => void }) {
  const [tab, setTab] = useState<'csv' | 'resumes'>('csv')
  ...
  return <Overlay onClose={onClose}>
    ...
    <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
      {(['csv', 'resumes'] as const).map((t) => <button key={t} onClick={() => { setTab(t); setResult(null) }} style={{ fontSize: 13, fontWeight: 600, padding: '6px 12px', borderRadius: 8, border: '1px solid ' + (tab === t ? '#4F46E5' : '#E2E8F0'), background: tab === t ? 'rgba(79,70,229,0.08)' : '#fff', color: tab === t ? '#4F46E5' : '#64748B', cursor: 'pointer' }}>{t === 'csv' ? 'CSV Import' : 'Resume Paste'}</button>)}
    </div>

    {result ? (
      <div>
        <div style={{ fontSize: 14, fontWeight: 700, color: '#10B981', marginBottom: 6 }}>✓ {result.created} imported · ✗ {result.failed} failed</div>
