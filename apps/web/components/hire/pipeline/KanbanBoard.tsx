...
const REC_BG: Record<string, string> = { strong_yes: '#ECFDF5', yes: '#EEF2FF', maybe: '#FFFBEB', no: '#FEF2F2' }
const REC_FG: Record<string, string> = { strong_yes: '#059669', yes: '#4F46E5', maybe: '#D97706', no: '#DC2626' }
...
function StageColumn({ stage, onCandidateClick }: { stage: KanbanStage; onCandidateClick: (c: KanbanCandidate) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={{ width: 240, flexShrink: 0, background: isOver ? '#F1F5F9' : '#F8FAFC', border: isOver ? '1px dashed #4F46E5' : '1px solid #E2E8F0', borderRadius: 10, padding: '12px 10px', minHeight: 200, transition: 'background 0.15s, border 0.15s' }}>
      ...
      {stage.candidates.length === 0 && <div style={{ fontSize: 12, color: '#CBD5E1', textAlign: 'center', paddingTop: 24 }}>Drop candidates here</div>}
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        ...
        <DragOverlay>
          {activeCandidate && (
            <div style={{ background: 'white', border: '1px solid #4F46E5', borderRadius: 8, padding: '10px 12px', width: 220, boxShadow: '0 8px 24px rgba(79,70,229,0.15)', opacity: 0.95 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#0F172A' }}>{activeCandidate.name}</div>
              <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>{activeCandidate.email}</div>
            </div>
          )}
        </DragOverlay>
      </DndContext>
